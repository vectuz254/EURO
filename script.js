...
// Each .grid-tile cycles through its own image list, crossfading between them.
(function initGridSlideshow() {
  const tiles = document.querySelectorAll('.grid-tile');
  if (!tiles.length) return; // not on this page
  ...
  tiles.forEach((tile, tileIndex) => {
    ...
    imgA.src = images[0];
    imgB.src = images[1] || images[0];
    imgA.className = 'tile-layer active';
    imgB.className = 'tile-layer';
    imgA.alt = caption;
    imgB.alt = caption;
    imgA.onerror = () => tile.classList.add('placeholder');

    const overlay = document.createElement('div');
    overlay.className = 'photo-overlay';
    overlay.innerHTML = `<span>${caption}</span>`;

    tile.appendChild(imgA);
    tile.appendChild(imgB);
    ...
    function nextSlide() {
      ...
      const showing = layers[activeIdx];
      const hidden  = layers[(activeIdx + 1) % 2];
    ...
    // Stagger each tile's start so the grid doesn't flip all at once
    const delay = tileIndex * 700;
    setTimeout(() => {
